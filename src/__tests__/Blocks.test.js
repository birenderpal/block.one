...
describe('Blocks effects tests', () => {
    ...
    const getBlock = jest.fn().mockImplementation(() => Promise.resolve(block.success))
    const getBlocks = jest.fn().mockImplementation(() => Promise.resolve(blocks.success))
    ...
    it('On button click change button text', async () => {
        ...
        const { queryByTestId, getByTestId } = render(<Blocks getBlock={getBlock} getBlocks={getBlocks} />)
        ...
        let blockList = await waitForElement(() => getByTestId("block-list"));
    ...
    it('On error should show error', async () => {
        ...
        let getBlock = jest.fn().mockImplementation(() => Promise.resolve(block.error))
        let getBlocks = jest.fn().mockImplementation(() => Promise.resolve(blocks.error))
    ...
    it('On block click should fetch block', async () => {
        ...
        const { queryByTestId, getByTestId, getAllByTestId } = render(<Blocks getBlock={getBlock} getBlocks={getBlocks} />)
        
        //click button
        fireEvent.click(getByTestId('load-button'))
        
        // get all the block shown
        let fetchedBlock = await waitForElement(() => getAllByTestId("clickable-block"));
        
        //click on the first block
        fireEvent.click(fetchedBlock[0])
        let rawBlock = await waitForElement(() => getAllByTestId("raw-block"));
