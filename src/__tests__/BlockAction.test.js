...
describe('<BlockAction>', () => {
    beforeEach(() => {
        wrapper = shallow(<BlockAction></BlockAction>)
    });
    afterAll(() => {
        wrapper.unmount()
    });
...
it('Should render children html elements', () => {
    wrapper = shallow(<BlockAction><div className="child"/></BlockAction>)
    expect(wrapper.find('.child')).to.have.length(1)
});

it('Should render children html elements', () => {
    wrapper = shallow(<BlockAction><div className="child"/><div className="child"/></BlockAction>)
